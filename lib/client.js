 ...
 *  - `password`, tender account password (for email, password login)
 *  - `userID`, ID of the user (for token login)
 *  - `defaultDevice`, (optional) default device to connect to
 *  - `devices`, (optional) dict of devices, {id:{hwid,name}}
 */
  ...
  if (options) {
    ...
    if (options.devices) {
      for (deviceID in options.devices) {
        this.devices[deviceID]=new Device(options.devices[deviceID])
      }
    }
  }
  var that=this;
  return callIf(!(this.userID && this.apiToken), this.auth)(options)
...
Client.prototype.listen = function listen(id) {
  ...
  if (this.devices.hasOwnProperty(id)) {
    if (this.listening.indexOf(id) === -1) {
      //add to list
      this.listening.push(id.toString())
...
Client.prototype._onConnect = function _onConnect() {
  ...
  this.connected=true;
  this.reconnectPeriod = constants.RECONNECT_PERIOD_MIN;
  var listenTo = this.listening.concat([this.defaultDevice])
  for (index in listenTo) {
    this._susbcribe(this.devices[listenTo[index]])
  }
};
