...
Client.prototype.connect = function connect() {
  if (!(this.userID && this.apiToken)) {
    this.auth(this.options, this.connect);
  }
 ...
 */
Client.prototype.auth = function auth(options, callback) {
  var authData = {};
  if (options.email && options.password) {
    authData = { email: options.email, password: options.password };
  } else {
    Promise.reject(
      new Error("Must provide email and password combo to authenticate")
    );
  }
  var request = {
    method: "POST",
  ...
  return fetch(api.routes.auth, request)
    ...
    .then(function(response) {
      if (response.hasOwnProperty("error")) {
        ...
        that.userID = response.user_id;
        that.apiToken = response.api_token;
